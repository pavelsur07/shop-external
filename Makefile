...
#------------------------------------ Test -----------------------------------------------
#test-init:
#	REGISTRY=localhost IMAGE_TAG=0 make test-up

#test-up:
#	docker-compose -f docker-compose-test.yml up -d
#	docker-compose -f docker-compose-test.yml run --rm site-php-cli wait-for-it market-api-postgres:5432 -t 60
#	docker-compose -f docker-compose-test.yml run --rm site-php-cli php bin/console doctrine:migrations:migrate --no-interaction
#	docker-compose -f docker-compose-test.yml run --rm stag-site-php-cli php bin/console doctrine:fixtures:load --no-interaction

#testing-down-clear:
#	docker-compose -f docker-compose-test.yml down -v --remove-orphans

#try-test-build: try-build try-staging-build
...
build-site:
	docker --log-level=debug build --pull --file=site/docker/production/elasticsearch/Dockerfile --tag=${REGISTRY}/elasticsearch:${IMAGE_TAG} site
